...
app.get('/board/detail.do', function(req, res) {
	connection.query(
	  "select bno, title, content, date_format(cre_date,'%Y-%m-%d') as cdate , views from board2 where bno=?", 
	  [req.params['no']],
	  function(err,result){
		if (err){
		  console.log(err);
		...
		res.write('번호: <input type="text" name="number" size="5" readonly value=" '
				+ result[0].bno+' "><br>\n');
		res.write('제목: <input type="text" name="title" size="50" value="'+result[0].title+'"><br>\n');
		res.write('내용: <textarea name="content" rows="6" cols="50">"'+result[0].content+'"</textarea><br>\n');
		res.write('등록일: '+result[0].cdate+'\n<br>');
		res.write('조회수: '+result[0].views+'\n<br>');
