...
<%@ page language="java" contentType="text/plain; charset=UTF-8"
	...
	제목 : ${board.title}
	    ...
	    c:if 는 있ㅈ지만 else가 없다.
	    [c:choosse]
	    여러개의 조건을 검사하고 싶을 때 사용한다.
	    if ~~ else 또는 switch 문과 유사하다.
	  	switch문과 유사
	  	<c:choose>
	  	<c:when test="${a>b }"> a는 b보다 크다</c:when>
	  	<c:when test="${a==b }">a는 b와 같다.</c:when>
	  	<c:otherwise>a는 b보다 작다</c:otherwise>
	    </c:choose>